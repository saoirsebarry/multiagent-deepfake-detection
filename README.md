...
100 clips from 25 public YouTube videos: BBC News bulletins (real), AI news anchors (Aaj Tak's Sana, OTV's Lisa) and AI-avatar showcases (HeyGen, Synthesia) and creator channels (fake). Every clip passed the per-frame gates of `youtube_eval/extract_clips.py`, its label follows the provenance of the face shown (verified per clip from contact sheets, full frames, video descriptions and transcripts), and the clip list and labels were frozen and hashed before any clip was scored (`youtube_eval/manifest.json`, `youtube_eval/clip_provenance.json`). Per-clip agent scores are in `paper_artifacts/source_csvs/analysis_results_youtube.csv`; regenerate the clips with `extract_clips.py` and score them with `youtube_eval/score_clips.py`. Raw videos are not redistributed (copyright).
...
## 7. Provenance of the headline numbers

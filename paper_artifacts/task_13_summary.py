...
def main() -> None:
    ...
    push(f"The real / fake `final_score` distributions are fully separable "
         f"(max real = 0.457 < min fake = 0.492), so AUC-ROC and AP are exactly "
         f"1.0. The 3 errors at τ = 0.5 are fake samples with scores in "
         f"(0.457, 0.500) (see Section 3).")
    push("")
    ...
    push("2. **Aggregation rule.** The CSV's `final_score` is a **weighted** mean with "
         ...
         "follows the same weighted rule so baseline = headline (3 errors). The "
         "weights are stated in the paper's Section 3.5 and selected on the validation partition.")
    push("")
    push("3. **Decision threshold.** τ = 0.50 throughout: the paper, the released "
         "orchestrators and this pipeline agree.")
    push("")
    push("4. **YouTube sample size.** 37 quality-gated clips with per-clip provenance labels.")
    push("")

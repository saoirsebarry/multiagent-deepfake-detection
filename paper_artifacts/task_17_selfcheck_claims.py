...
check("errors at tau=0.5", float(((agg >= TAU).astype(int) != y).sum()), 1.0, 0)
check("false negatives at tau=0.5", float(((agg < TAU) & (y == 1)).sum()), 0.0, 0)
check("separation margin (min fake - max real)", float(agg[y == 1].min() - agg[y == 0].max()), -0.0172, 5e-4)
from sklearn.metrics import roc_auc_score, average_precision_score
check("AUC-ROC", float(roc_auc_score(y, agg)), 0.999975, 5e-6)
check("average precision", float(average_precision_score(y, agg)), 0.999999, 2e-6)

print("\ncorrelations (section 4.3)")
Cs, Ce = np.corrcoef(S.T), np.corrcoef(err.T)
off = lambda C: float(np.max(C - np.eye(len(C))))
check("max SCORE correlation (cross-modal / biometric)", off(Cs), 0.936, 1e-3)
check("max ERROR correlation (visual / biometric)", off(Ce), 0.287, 1e-3)
check("cross-modal / ECAPA error correlation", float(Ce[3, 2]), -0.010, 1e-3)

print("\nthree-agent Phase-1 configuration (sections 4.1, 4.4)")
w3 = np.array([W5[2], W5[3], W5[4]]); w3 = w3 / w3.sum()
S3 = S[:, [2, 3, 4]]
p1 = S3 @ w3
check("renormalised Phase-1 errors (Tables 9, 11, 13)", float(((p1 >= TAU).astype(int) != y).sum()), 3.0, 0)
check("renormalised Phase-1 false negatives", float(((p1 < TAU) & (y == 1)).sum()), 1.0, 0)
...
check("Phase-1 score spread (std)", float(trio.std()), 0.005, 1e-3)
...
uni = S @ np.full(5, 0.2)
check("uniform weighting errors at tau=0.5", float(((uni >= TAU).astype(int) != y).sum()), 0.0, 0)
check("uniform weighting AUC", float(roc_auc_score(y, uni)), 1.0, 1e-9)
...
check("validation margin", float(aggv[yv == 1].min() - aggv[yv == 0].max()), 0.1106, 5e-4)
check("validation max real (band low side)", float(aggv[yv == 0].max()), 0.4335, 5e-4)
check("validation min fake (band high side)", float(aggv[yv == 1].min()), 0.5441, 5e-4)

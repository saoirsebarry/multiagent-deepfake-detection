...
REPO = "/content/repo"
WORK = "/content/work"
RETRAIN = os.path.join(DRIVE, "retrain")
LOGDIR = os.path.join(DRIVE, "training_logs")
MARKERS = os.path.join(RETRAIN, "markers")
...
if not done("1_test_preprocessed"):
    ...
    run([sys.executable, "-u", "src/data_preprocessing/preprocessed_all_unbalanced.py",
        ...
        cwd=REPO, logname="retrain_preprocess_test.log")
...
if not done("1b_local_copy"):
    for split in ("train", "val", "test"):
        ...
        run(["rsync", "-a", "--info=stats1", src + "/", dst + "/"],
            cwd="/content", logname="retrain_localcopy.log")
...
if not done("2_xception"):
    ...
    xdir = os.path.join(RETRAIN, "xception")
...
if not done("5_scored"):
    ck = os.path.join(WORK, "repo_retrain")
    ...
    pairs = [
        (os.path.join(RETRAIN, "xception", "polyglotfake_xception_best_pytorch_unbal_all.pth"),
         "checkpoints/xception/polyglotfake_xception_best_unbal_all_faceaug.pth"),
        (os.path.join(RETRAIN, "freqnet", "freqnet_model_all_unbalanced_improved.pth"),
         "checkpoints/freqnet/freqnet_model_all_unbalanced_improved.pth"),
        (os.path.join(RETRAIN, "crossmodal", "lip_sync_model_crossattention_all_unbalanced.pth"),
         "checkpoints/cross_modal/lip_sync_model_crossattention.pth"),
        (os.path.join(LOGDIR, "ecapa", "audio_forensics_model_best.pth"),
         "checkpoints/ecapa_forensic_head/audio_forensics_model_finetuned_best.pth"),
        (os.path.join(LOGDIR, "ecapa", "training_stats.npz"),
    ...
    out_csv = os.path.join(RETRAIN, "analysis_results_retrain_test.csv")
    run([sys.executable, "-u", "src/orchestrator.py", "--split", "test", "--output_file", out_csv],
        cwd=ck, logname="retrain_orchestrator_test.log")
    mark("5_scored")
...
TAU = 0.37
df = pd.read_csv(os.path.join(RETRAIN, "analysis_results_retrain_test.csv"))
...
out = os.path.join(RETRAIN, "seed_replicate_test.json")

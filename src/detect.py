...
Cross-Modal (Lip-Sync), and Face Quality agents with explainable outputs
"""
...
# Get model architectures from the multiagent_models folder
...
CONFIG = {
    ...
    "model_files": {
        ...
        "audio": "checkpoints/freqnet/freqnet_model_finetuned_robust.pth",
        "audio_forensics": "checkpoints/ecapa_forensic_head/audio_forensics_model_finetuned_robust.pth",
        "cross_modal": "checkpoints/cross_modal/lip_sync_model_crossattention_finetuned_robust.pth",
        "face_quality": "checkpoints/biometric/best_model.pth",
    },
    "audio_forensics_stats_path": "checkpoints/ecapa_forensic_head/training_stats_finetuned_robust.npz",
...
class MultiAgentOrchestrator:
    ...
    def __init__(self, models: Dict[str, Any]):
        ...
        # Agent functions with XAI capabilities
    ...
    def orchestrate_analysis(self, media_data: Dict[str, Any]) -> Dict[str, Any]:
        ...
        # Deploy additional agents if disagreement > threshold
        ...
        # update communication hub and return final decision
    ...
    def _calculate_disagreement(self, results: Dict[str, Any]) -> float:
        ...
        # disagreement is 0 if there are two agents
        if len(valid_scores) < 2:
            return 0.0  
        
        # disagreement is calculated as the standard deviation of the score
        ...
        if len(set(verdicts)) > 1:
            # check if disagreement is minimum 0.3
    ...
    def _finalize_decision(self, results: Dict[str, Any], start_time: float) -> Dict[str, Any]:
        ...
        # pool together the visualisation results
        visualizations = []
...
def run_visual_analysis(media_data: Dict[str, Any], models: Dict[str, Any]) -> Dict[str, Any]:
    ...
    try:
        ...
        # --- PLOTTING LOGIC MODIFIED BELOW ---

        # CHANGED: The number of frames to plot is now 10.
        MAX_FRAMES_TO_PLOT = min(10, len(faces))
        
        # CHANGED: Create N+1 subplots to accommodate the original image.
        fig, axs = plt.subplots(1, 1 + MAX_FRAMES_TO_PLOT, figsize=((1 + MAX_FRAMES_TO_PLOT) * 4, 4))

        # CHANGED: Plot the original, non-overlaid first frame in the first position.
        axs[0].imshow(faces[0])
        axs[0].set_title("Original")
        axs[0].axis('off')

        # CHANGED: Loop through frames and plot overlays in the subsequent positions (axs[i+1]).
...
def run_cross_modal_analysis(media_data: Dict[str, Any], models: Dict[str, Any]) -> Dict[str, Any]:
    ...
    # --- CHECKPOINT 2: Visual Tensor Preparation ---
    ...
    # --- CHECKPOINT 3: Audio Tensor Preparation ---
    ...
    try:
        ...
        # --- CHECKPOINT 5: Analyzing Attention Weights ---
...
def run_face_quality_analysis(media_data: Dict[str, Any], models: Dict[str, Any]) -> Dict[str, Any]:
    ...
    try:
        ...
        # --- Select the middle frame for the main analysis ---
...
"""
    ...
    try:
        ...
        logging.info(f"Generating forensic report")
...
def main():
    ...
    try:
        ...
        if media_data_dict:
            ...
            # add semantic results to the decision dictionary
